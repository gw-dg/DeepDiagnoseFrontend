...
function DiseaseCard({ predictions }: DiseaseCardProps) {
  const cleanArrayString = (str: string | undefined): string[] => {
    if (!str) return [];
              ...
              <Tabs defaultValue="precautions" className="w-full">
                ...
                <TabsContent value="precautions" className="mt-4">
                  <ul className="list-disc pl-5 space-y-1">
                    {prediction.Precautions.slice(1).map(
                ...
                <TabsContent value="diet" className="mt-4">
                  <ul className="list-disc pl-5 space-y-1">
                    {cleanArrayString(prediction.Diet[0]).map(
                      (diet: string, i: number) => (
                        <li key={i} className="text-sm">
                          {diet}
                        </li>
                      )
